  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      ...
      try {
        ...
        console.log('🔐 [AuthContext] Saved user from localStorage:', savedUser ? savedUser.name : 'null');
        ...
        if (savedUser && (savedUser.uid.startsWith('demo_') || savedUser.uid.startsWith('guest_') || savedUser.uid.startsWith('user_'))) {
          console.log('🔐 [AuthContext] Found demo/guest user in localStorage, using it immediately');
          if (mounted) {
            setUser(savedUser);
        ...
        console.log('🔐 [AuthContext] Supabase configured:', useSupabase);
        ...
        if (useSupabase && supabase) {
          ...
          console.log('🔐 [AuthContext] Supabase session:', session?.user?.id || 'No session');
          ...
          if (session?.user) {
            try {
              ...
              if (!profile) {
                console.log('🔐 [AuthContext] Creating missing profile for:', session.user.id);
              ...
              if (profile && mounted) {
                ...
                console.log('🔐 [AuthContext] Profile still null after creation attempt');
              }
            } catch (profileError) {
            ...
            console.log('🔐 [AuthContext] Using saved user from localStorage:', savedUser.name);
            setUser(savedUser);
          }
          ...
          const { data } = supabase.auth.onAuthStateChange(async (event, session) => {
            ...
            if (event === 'SIGNED_IN' && session?.user) {
              try {
                ...
                if (!profile) {
                  console.log('🔐 [AuthContext] Creating profile on SIGNED_IN');
                  await createUserProfile(session.user.id, {
                    email: session.user.email,
                    name: session.user.user_metadata?.name || 'User',
                  });
                ...
                console.error('🔐 [AuthContext] Error in SIGNED_IN:', e);
              }
          ...
          if (savedUser && mounted) {
            console.log('🔐 [AuthContext] Demo mode: Using saved user:', savedUser.name);
            setUser(savedUser);
          } else {
            console.log('🔐 [AuthContext] Demo mode: No saved user found');
          }
          if (mounted) setLoading(false);
        }
      } catch (error) {
        ...
        const savedUser = getSavedUser();
        if (savedUser && mounted) {
          console.log('🔐 [AuthContext] Using saved user as fallback:', savedUser.name);
          setUser(savedUser);
        }
        if (mounted) setLoading(false);
      }
